...
update_params({
    ...
    'Axial Reflector Thickness': 20.0,             # cm
})
#params['Assembly FTF'] = (params['Lattice Pitch'] * (params['Number of Rings per Assembly'] - 1) + 1.4 * params['Fuel Pin Radii'][-1]) * np.sqrt(3)
params['Assembly FTF'] = (params['Lattice Pitch'] * (params['Number of Rings per Assembly'] - 1) * np.sqrt(3) + 2.0 * params['Fuel Pin Radii'][-1] + params['Pin Gap Distance'])
...
params['Radial Reflector Thickness'] = (params['Assembly FTF'] / 2.0)
#params['Core Radius'] = 0.5*np.sqrt(3)*params['hexagonal Core Edge Length'] + params['Radial Reflector Thickness']
params['Core Radius'] = (params['Assembly FTF'] * params['Number of Rings per Core'] + params['Radial Reflector Thickness'])
#params['Active Height'] = 2 * params['Core Radius'] #cm  
params['Active Height'] = ((10.0 / 7.0) * params['Core Radius'])
...
update_params({
    ...
    'Drum Radius': params['Core Radius'] / 7.442, #0.4 * params['Radial Reflector Thickness'], 
...
# print("\n" + "=" * 70)
# print("REFERENCE rHPMR - BEGINNING-OF-LIFE PERFORMANCE")
# print("=" * 70)

# print(f"Fuel                     : {params['Fuel']}")
# print(f"Thermal Power            : {params['Power MWt']:.3f} MWth")
# print(f"Enrichment               : {100 * params['Enrichment']:.3f} wt.%")
# print(f"Fuel Radius              : {params['Fuel Pin Radii'][0]:.3f} cm")
# print(f"Active Height            : {params['Active Height']:.3f} cm")

# print(f"Initial U-235 Mass       : {params['Mass U235']/1000:.4f} kg")
# print(f"Initial U-238 Mass       : {params['Mass U238']/1000:.4f} kg")
# print(f"Total Uranium Mass       : {params['Uranium Mass']:.4f} kg")

# print(f"BOL keff (2D)            : {params['BOL keff 2D']:.5f}")
# print(f"BOL Correction Factor    : {params['BOL Correction Factor']:.5f}")
# print(f"BOL Corrected keff       : {params['BOL Corrected keff']:.5f}")
# print(f"BOL Axial Leakage        : {params['BOL Axial Leakage (%)']:.3f} %")

# print(f"BOL Peaking Factor       : {params['BOL Peaking Factor']:.4f}")
# print(f"BOL Peak Region          : {params['BOL PF Region']}")
...
# print("\n" + "=" * 70)
# print("rHPMR GEOMETRY CHECK")
# print("=" * 70)

# print(f"Assembly FTF                  : "f"{params['Assembly FTF']:.3f} cm")

# print(f"Radial Reflector Thickness    : "f"{params['Radial Reflector Thickness']:.3f} cm")

# print(f"Core Radius                   : "f"{params['Core Radius']:.3f} cm")

# print(f"Active Height                 : "f"{params['Active Height']:.3f} cm")

# print(f"Axial Reflector Thickness     : "f"{params['Axial Reflector Thickness']:.3f} cm each side")

# print(f"Total Axial Height            : "f"{params['Active Height'] + 2 * params['Axial Reflector Thickness']:.3f} cm")

# print(f"Drum Radius                   : "f"{params['Drum Radius']:.3f} cm")

# print(f"Drum Height                   : "f"{params['Drum Height']:.3f} cm")
...
# Reference geometry
print(f"Assembly FTF                 : {params['Assembly FTF']:.3f} cm")
print(f"Core Radius                  : {params['Core Radius']:.3f} cm")
print(f"Active Height                : {params['Active Height']:.3f} cm")
print(f"Radial Reflector Thickness   : {params['Radial Reflector Thickness']:.3f} cm")
print(f"Axial Reflector Thickness    : {params['Axial Reflector Thickness']:.3f} cm")
print(f"Drum Radius                  : {params['Drum Radius']:.3f} cm")
